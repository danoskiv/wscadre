	...
	if($hours == '1')
	{
		...
		{		
			...
			curl_setopt_array($curl, array(
			    ...
			    CURLOPT_URL => 'http://air.moepp.gov.mk/graphs/site/pages/MakeGraph.php?station=WesternRegion&parameter='.$parametri[$j].'&beginDate='.$beginDate.'&beginTime='. $beginHour .'&endDate='.$endDate.'&endTime='. $endHour .'&i=1521227934243&lang=en',
...
else
{
	...
	curl_setopt_array($curl, array(
	    CURLOPT_RETURNTRANSFER => 1,
	    CURLOPT_URL => 'http://air.moepp.gov.mk/graphs/site/pages/MakeGraph.php?station=WesternRegion&parameter='.$parametri[$j].'&beginDate='.$beginDate.'&beginTime=00:00&endDate='.$endDate.'&endTime=00:00&i=1521227934243&lang=en',
	    CURLOPT_USERAGENT => 'Codular Sample cURL Request'
	));
	// Send the request & save response to $resp
	$resp = curl_exec($curl);
